...
import { avrDataSchema } from "@/lib/documents/schemas";
import { EMPTY_REQUISITES } from "@/lib/documents/types";
import { generateAvrHtml } from "@/lib/documents/avr-template";
...
function buildFilename(documentNumber: string): string {
  ...
  return `sanaai_avr_${documentNumber || "1"}_${dd}${mm}${yyyy}.pdf`;
}
...
export async function POST(request: Request) {
  ...
  const parsed = avrDataSchema.safeParse(json);
  if (!parsed.success) {
  ...
  const executorName = data.executorName || requisites.companyName;
  const executorAddress = data.executorName ? "" : requisites.address;
  ...
  const html = generateAvrHtml({
    ...
    items: data.items,
    reservesInfo: data.reservesInfo,
    attachmentNote: data.attachmentPages,
  ...
  return new NextResponse(new Uint8Array(pdf), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${buildFilename(data.documentNumber)}"`,
    },
  });
}
