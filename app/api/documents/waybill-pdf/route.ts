...
import { waybillDataSchema } from "@/lib/documents/schemas";
import { EMPTY_REQUISITES } from "@/lib/documents/types";
import { generateWaybillHtml } from "@/lib/documents/waybill-template";
...
function buildFilename(documentNumber: string): string {
  ...
  return `sanaai_waybill_${documentNumber || "1"}_${dd}${mm}${yyyy}.pdf`;
}
...
export async function POST(request: Request) {
  ...
  const parsed = waybillDataSchema.safeParse(json);
  if (!parsed.success) {
